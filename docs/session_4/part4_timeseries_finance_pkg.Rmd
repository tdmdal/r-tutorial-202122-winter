...
4. many more classes such as `timeSeries` in [timeSeries](https://cran.r-project.org/web/packages/timeSeries/timeSeries.pdf) package, `tis` in [tis](https://cran.r-project.org/web/packages/tis/tis.pdf) package, etc.
...
ACF - Autocorrelation Function. ACF calculates the correlation of a time series with its own lagged values. (see [here](https://nwfsc-timeseries.github.io/atsa-labs/sec-ts-correlation-within-and-among-time-series.html#sec-ts-autocorrelation-function-acf))
...
[txbox](https://www.tsbox.help/) provides conversion between many time series data structures, an attempt to unite time series data structure in R.
